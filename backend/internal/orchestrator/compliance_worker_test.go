...
func TestComplianceWorker_SuggestsRGPDReport(t *testing.T) {
	store := &memStore{}
	...
	orc := orchestrator.New(bus, nil, orchestrator.NewComplianceWorker(bus))
	orc.Start()
	...
	payload, _ := json.Marshal(map[string]any{"invoice_id": "inv-1", "reason": "erp_flow_complete"})
	_ = bus.Publish(ctx, eventbus.Event{Type: eventbus.HRComplianceRequested, UserID: "u1", Payload: payload})
	...
	if body["action"] != "generate_rgpd_report" {
